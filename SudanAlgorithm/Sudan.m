...
for i=1:size(polynomials,2),
	new_factors = Factorise_gf(polynomials(:,i),m,k,x_limit,y_limit);
...
for i=1:size(factors,2),
    [msg factor] = Factor_Exhaust(x_mat,y_mat,factors(:,i),m,t,x_limit,y_limit);
    List = [List msg];
...
fprintf('');
[Min_Value, Min_Factor] = Minimum_Distance(List,Factor_List,Y);
end
